...
class Validate extends \Prefab
{
     ...
     */
    public static function is_valid(array $data, array $validators, $ucfield = true)
    {
        $instance = self::instance();

        $instance->validation_rules($validators);

        if ($instance->run($data) === false) {
            return $instance->get_error_array($ucfield);
        } else {
            return true;
        }
    }
    ...
    public static function filter_input(array $data, array $filters)
    {
        ...
        return $instance->filter($data, $filters);
    }
     ...
     * @throws Exception
     */
    public function add_cortex($model)
    {
        if($model instanceof \DB\Cortex){
            $fields = $this->cortex_fields($model);
            $filters = array();
            $validations = array();
            foreach($fields as $key=>$field){
                ...
                if($field['validate'])
                    $validations[$key] = $field['validate'];
            }
    ...
    private function cortex_fields( &$Instance ) {
        $clone = (array) $Instance;
        $rtn = array ();
        $rt['___SOURCE_KEYS_'] = $clone;


        while ( list ($key, $value) = each ($clone) ) {
            $aux = explode ("\0", $key);
            $newkey = $aux[count($aux)-1];
            $rtn[$newkey] = $rt['___SOURCE_KEYS_'][$key];
        }

        return $rtn['fieldConf']?$rtn['fieldConf']:[];
    }
     ...
     * @throws Exception
     */
    public function validates(array $input, array $ruleset)
    {
        $this->errors = array();
     ...
     * Usage:
     *
     * Validate::set_field_names(array(
     * 	"name" => "My Lovely Name",
     ...
     */
    public function get_error_array($ucfield = true)
    {
        $app = Base::instance();
        ...
        foreach ($this->errors as $e) {
            ...
            // get error language
            $err = $app->get($app->PREFIX . 'VALIDATE.' . str_replace('validate_', '', $e['rule'] ?: 'fail'));
            
            // modify error parameter
            switch ($e['rule']) {
                case 'validate_contains':
                    $param = implode(', ', $param);
                    break;
                case 'validate_equalsfield':
                    $param = ucwords(str_replace(array('_', '-'), chr(32), $param));
                    break;
            }
            ...
            // add field error
            $resp[$field] = Preview::instance()->resolve($err, ['field' => $field, 'param' => $param]);
        }
    ...
    */
     ...
     * @return mixed
     */
    protected function validate_email($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }

        if (!Audit::instance()->email($input[$field],false)) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
